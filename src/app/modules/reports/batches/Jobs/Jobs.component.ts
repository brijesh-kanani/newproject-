...
@Component({
    ...
    standalone: true,
    imports: [CommonModule, MatIconModule, MatTableModule, MatPaginatorModule, MatSortModule, MatMenuModule, MatSnackBarModule, JobsComponent
        , MatExpansionModule,
        CommonModule,
        FormsModule,
        ReactiveFormsModule,
...
export class JobsComponent implements OnInit {
    ...
    viewMappingData: any = []
    displayedColumns: string[] = ['fileNameOnly', 'customer', 'AccountName', 'JobTypeId', 'action'];
    customerList: any = [];
    filteredData: any = [];
    ...
    filterBatchList: any;
    constructor(private snack: MatSnackBar, private dialog: MatDialog, private _router: Router, private _spinner: NgxSpinnerService, private sharedService: SharedService, public batchesService: BatchesService, private importService: ImportService, private cdr: ChangeDetectorRef, private fileLogService: FileLogService, private fb: FormBuilder, private location: Location, private customerAccountService: CustomerAccountService) {
    ...
    private _filterAccountName(name: string): any[] {
        ...
        return this.accountList.filter(option => option.AccountName.toLowerCase().includes(filterValue.toLowerCase()));
    }

    private _filterBatchName(name: string): any[] {
        const filterValue = name.toLowerCase();
        return this.batchList.filter(option => option.Name.toLowerCase().includes(filterValue.toLowerCase()));
    }
    ...
    async getAccountList() {
        ...
        await this.customerAccountService.getAccountList(filter).then((response) => {
            if (response) {
                ...
                this.filterAccountList = this.searchAccount.valueChanges
                    .pipe(
                        startWith<string | any>(''),
                        map(value => typeof value === 'string' ? value : value),
                        map(name => name ? this._filterAccountName(name) : this.accountList.slice()),
                    );
                setTimeout(() => {
                    this._spinner.hide()
                }, 500);
    ...
    async getBatchList() {
        this._spinner.show()
        let data: any = []
        this.batchesService.getBatchData().then(async (response) => {
    ...
    async applyFilters() {
        ...
        let filter = {
            batcheName: this.filterForm.value.name || '', type: this.filterForm.value.type || '', account: this.filterForm.value.account || '', customerName: this.filterForm.value.customerName || '', selectBatch: this.filterForm.value.selectBatch || '', page: page, pageSize: pageSize
        }
    ...
    async setJobList() {
        console.log(this.batchesService.storeData, 'this.batchesService.storeData')
        this._spinner.show()
        if (this.batchesService.storeData?.JobFiles) {
            ...
            this.totalItemCount = this.batchesService.storeData.length > 0 && this.batchesService.storeData.length || 500
            this.filterForm.patchValue({ selectBatch: this.batchesService.storeData.BatchId })
        }
        this._spinner.hide()
        this.applyFilters()
    }
