...
@Component({
  selector: 'app-report-log-filter',
  templateUrl: './report-log-filter.component.html',
  styleUrls: ['./report-log-filter.component.scss'],
  standalone: true,
  imports: [
    MatCommonModule,
    FormsModule,
...
export class ReportLogFilterComponent {
  ...
  async getAccountList() {
    ...
    await this.customerAccountService.getAccountList(filter).then((response) => {
      if (response) {
        ...
        if (this.localUser && this.accountList && this.localUser.IdRole == 2) {
  ...
  checkExitAccount() {
    ...
    //   this.filterForm.patchValue({ account: this.fileLogService.filters?.account })

    // }
    // if (this.fileLogService.filters && this.fileLogService.filters.fromDate) {
    //   this.filterForm.patchValue({ fromDate: this.fileLogService.filters?.fromDate })

    // }
    // if (this.fileLogService.filters && this.fileLogService.filters.toDate) {
    //   this.filterForm.patchValue({ toDate: this.fileLogService.filters?.toDate })
    // }
    // this.applyFilters()
  }
